...
class GameObject
{
	public:
		float GetLifeTime();
		GameObject(const std::string& pName = nullptr, const std::string& pTag = "Untagged", const glm::vec3& pPosition = glm::vec3(0.0f, 0.0f, 0.0f), bool visible = true);
		virtual ~GameObject();

		void StartsCollisionsWith(GameObject* obj);
		void StopsCollisionsWith(GameObject* obj);
		GameObject* CollidesWith(std::string typeorname);
		GameObject* CollisionEnter(std::string typeorname);
		...
		GameObject* find(std::string pName);

		void SetCollider(Collider* collider);
		Collider* GetCollider();
		bool IsVisible();
		void ChangeVisible(bool val);
		void ChangeVisible();
		...
		//void AddToWorld(GameObject* pGameObject);
		World* GetWorld();
	...
	protected:
		...
		std::set<GameObject*> _prevNameCols;
