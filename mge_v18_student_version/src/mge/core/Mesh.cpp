...
std::map<std::string, Mesh*>* Mesh::_cachedMeshes = new std::map<std::string, Mesh*>();

Mesh::Mesh(): _indexBufferId(0), _vertexBufferId(1), _normalBufferId(2), _uvBufferId(3), _tangentBufferId(4), _bitangentBufferId(5),  _vertices(), _normals(), _uvs(), _indices(),_tangents(), _bitangents()
{
	//ctor
}
...
{
	auto find = _cachedMeshes->find(pFilename);
	if (find == _cachedMeshes->end()) {
		...
		if (file.is_open()) {
			...
			file.close();
			/**/
			//creating tangents and bitangents
			int size = mesh->_vertices.size();

			for (int i = 0; i < mesh->_indices.size() ; i += 3) {
				//std::cout << i << " " << mesh->_indices.size() << " " << mesh->_vertices[mesh->_indices[i]] << mesh->_vertices.size() << std::endl;
				// Shortcuts for vertices
				glm::vec3 & v0 = mesh->_vertices[mesh->_indices[i] + 0];
				glm::vec3 & v1 = mesh->_vertices[mesh->_indices[i+1]];
				glm::vec3 & v2 = mesh->_vertices[mesh->_indices[i+2]];

				// Shortcuts for UVs
				glm::vec2 & uv0 = mesh->_uvs[mesh->_indices[i]];
				glm::vec2 & uv1 = mesh->_uvs[mesh->_indices[i+1]];
				glm::vec2 & uv2 = mesh->_uvs[mesh->_indices[i+2]];
			...
			/**/
			mesh->_buffer();
			_cachedMeshes->insert(std::make_pair(pFilename, mesh));
		...
		// Use Existing Mesh
		std::cout << "Loading cached mesh instead of creating new one. Name: " << pFilename << std::endl;
		Mesh* mesh = find->second;
		return mesh;
	}
}
...
void Mesh::_buffer()
{
	...
	glBufferData(GL_ARRAY_BUFFER, _tangents.size() * sizeof(glm::vec2), &_tangents[0], GL_STATIC_DRAW);

	glGenBuffers(1, &_bitangentBufferId);
	glBindBuffer(GL_ARRAY_BUFFER, _bitangentBufferId);
	glBufferData(GL_ARRAY_BUFFER, _bitangents.size() * sizeof(glm::vec2), &_bitangents[0], GL_STATIC_DRAW);
	/**/
