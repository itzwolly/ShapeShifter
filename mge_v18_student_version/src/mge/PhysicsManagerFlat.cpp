...
std::list<sf::RectangleShape>* PhysicsManagerFlat::_buttons = new std::list<sf::RectangleShape>();
...
PhysicsManagerFlat::PhysicsManagerFlat(bool show) :_showButtonAreas(show)
{
	_ends = new std::list<sf::Vector2f*>();
	_lines = new std::list<LineSegmentFlat*>();
	_buttons = new std::list<Button*>();
	_unMovables = new std::list<sf::RectangleShape>();
	//_showLines = false;
}

PhysicsManagerFlat::~PhysicsManagerFlat()
{
}
...
void PhysicsManagerFlat::ShowButtonAreas(bool val)
{
	_showButtonAreas = val;
}
...
void PhysicsManagerFlat::RegisterButton(Button* obj)
{
	_buttons->push_back(obj);
}
...
void PhysicsManagerFlat::Update(sf::RenderWindow* wind)
{
	...
	for (std::list<Button*>::iterator it = _buttons->begin(); it != _buttons->end(); ++it)
	{
		Button* button = it._Ptr->_Myval;
		sf::RectangleShape* rec = button->GetRectangle();
		sf::Vector2f recPos = rec->getPosition();
		sf::Vector2i mousePos = sf::Mouse::getPosition() - wind->getPosition()-sf::Vector2i(10,44);
		//std::cout << wind->getPosition().x << ", " << wind->getPosition().y << "\n";
		//std::cout << mousePos.x << ","<<mousePos.y << " - "<<recPos.x<<","<<recPos.y<<"\n";
		/**/
		if (button->IsClickable())
		{
			if (mousePos.x > recPos.x  && mousePos.x<recPos.x + rec->getSize().x && mousePos.y > recPos.y  && mousePos.y < recPos.y + rec->getSize().y)
			{
				if (clicked)
				{
					button->Clicked();
				...
				//std::cout << "in button";
				rec->setFillColor(it._Ptr->_Myval->GetColorPressed());
				button->Press(true);
			}
			else
			{
				button->Press(false);
				rec->setFillColor(it._Ptr->_Myval->GetColorNotPressed());
			}
		}
		/**/
	}
	...
	if (_showButtonAreas)
	{
		for (std::list<Button*>::iterator it = _buttons->begin(); it != _buttons->end(); ++it)
		{
			//it._Ptr->_Myval.setPosition(100, 100);
			wind->draw(*it._Ptr->_Myval->GetRectangle());
		}
	}

	/*if (_showLines)
	{
		for (std::list<LineSegmentFlat*>::iterator it = _lines->begin(); it != _lines->end(); ++it) {
