...
int actualLighNr = 0;
	vec4 color=vec4(0,0,0,0);
  	for (int i = 0; i < lightNr; i++) {
		...
		vec4 tempColor = vec4 ((diffuseTerm+ambientTerm)*lightIntensity , transparency) + vec4 (specularLight*lightIntensity , 0);
		color += tempColor;
	...
	fragment_color = texture(textureT,texCoord)*color/actualLighNr;
}
